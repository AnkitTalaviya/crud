...
export function InventoryTable({ items, canManage, onView, onEdit, onDelete }) {
  return (
        ...
        <table className="min-w-full divide-y divide-[color:rgb(var(--border))]">
          <thead className="bg-slate-50/80 dark:bg-slate-900/70">
            <tr className="text-left text-xs uppercase tracking-[0.22em] text-slate-400">
              {['Item', 'Supplier / PO', 'Stock', 'Value', 'Updated', 'Actions'].map((heading) => (
            ...
            {items.map((item) => (
              <tr key={item.id} className="transition hover:bg-sky-500/[0.04]">
                ...
                <td className="px-6 py-5 text-sm text-slate-600 dark:text-slate-300">
                  <p>{item.supplier}</p>
                  <p className="mt-1 text-xs text-slate-400">{item.purchaseOrderNumber || 'No PO linked'}</p>
                </td>
                <td className="px-6 py-5 text-sm">
                  <p className="font-semibold">{item.quantity} units on hand</p>
                  <p className="mt-1 text-xs text-slate-400">
                    {item.quantityOnOrder ?? 0} on order / reorder at {item.reorderLevel}
                  </p>
                </td>
